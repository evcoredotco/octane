...
func Test_csmsEnqueuesChangeAvailability_stashesCallID(t *testing.T) {
	...
	val, ok := state.Pop(
		"ocpp16:csms_call:" + stationHandle + ":ChangeAvailability",
	)
	if !ok {
		t.Fatal(
			"csmsEnqueuesChangeAvailability: want stashed uniqueID, got nothing",
		)
	}
...
func Test_csmsEnqueuesChangeAvailability_errorOnWrongConnector(t *testing.T) {
	...
	if err == nil {
		t.Error(
			"csmsEnqueuesChangeAvailability: want error for wrong connectorId, got nil",
		)
	}
}
...
func Test_stationRespondsToChangeAvailability_sendsCALLRESULT(t *testing.T) {
	...
	respPayload := requireSentCallResultPayload(
		t,
		station,
		"stationRespondsToChangeAvailability",
	)
