...
func Test_csmsEnqueuesUnlockConnector_stashesCallID(t *testing.T) {
	...
	val, ok := state.Pop(
		"ocpp16:csms_call:" + stationHandle + ":UnlockConnector",
	)
	if !ok {
		t.Fatal(
			"csmsEnqueuesUnlockConnector: want stashed uniqueID, got nothing",
		)
	}
...
func Test_csmsEnqueuesUnlockConnector_errorOnWrongConnector(t *testing.T) {
	...
	if err == nil {
		t.Error(
			"csmsEnqueuesUnlockConnector: want error for wrong connectorId, got nil",
		)
	}
}
...
func Test_stationRespondsToUnlockConnector_sendsUnlocked(t *testing.T) {
	...
	respPayload := requireSentCallResultPayload(
		t,
		station,
		"stationRespondsToUnlockConnector",
	)
